...
import cn.sunshine.component.common.domain.TreeNode;
...
public class TreeUtil {
	...
	public  <T extends TreeNode> List<T> buildTree(List<T> list) {
		...
		if(list != null && list.size() > 0) {
            ...
            for (T TreeBean : list) {
				if(TreeBean.getPid() == null || StrUtilExt.isEmpty(TreeBean.getPid())) {
					//节点的父类id为空则表示自己是顶层节点
					topList.add(TreeBean);
				}else {
					//如果节点的父类id不在所有节点的id中，则表示是顶层节点，否则是非顶层节点
					if(StrUtilExt.isEmpty(idMap.get(TreeBean.getPid()))) {
						topList.add(TreeBean);
					}else {
